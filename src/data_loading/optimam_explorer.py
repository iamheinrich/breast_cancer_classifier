...
class OptimamExplorer:
    ...
    def save_dicom_as_png(
        ...
        """
        Convert DICOM mammogram to PNG format.
        
        Args:
            dicom_path: Path to input DICOM file
            png_path: Path to output PNG file
            bitdepth: Bit depth of the input image (default: 12 for mammograms)
        """
        try:
            dicom = pydicom.read_file(str(dicom_path))
            image = dicom.pixel_array
            
            # Normalize pixel values
            max_pixel_value = np.max(image)
            scale_factor = (2 ** bitdepth - 1) / max_pixel_value
            image = (image * scale_factor).astype(np.uint16)
    ...
    def process_exams(self, num_exams: int) -> None:
        """
        Process a specified number of exams and save results.
...
def main():
    ...
    explorer.process_exams(num_exams=2)
