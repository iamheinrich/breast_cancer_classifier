...
## Project Overview and Data Integration (Adapted Version)

This repository contains an adapted version of the NYU Breast Cancer Classifier. Key modifications have been made to enable the model training and evaluation pipeline to run with custom datasets. Specifically, this version demonstrates compatibility with:

- **OMI-DB (Optimam Mammography Image Database):** [Further Information](https://www.cancerresearchhorizons.com/our-portfolio/our-licensing-opportunities/optimam-mammography-image-database-omi-db)
- **VinDr-Mammo (A large-scale benchmark dataset for computer-aided diagnosis in full-field digital mammography):** [Further Information & Download](https://vindr.ai/datasets/mammo)

The `run.sh` script in this repository provides an end-to-end example of the processing pipeline (cropping, optimal center extraction, heatmap generation, and model inference).

**Key Changes from the Original NYU `run.sh`:**

- The original script was configured to use `sample_data/` for inputs and `sample_output/` for all generated files.
- This adapted `run.sh` script is now configured by default to use the `sample_data_omi_db/` directory for both input data and all intermediate/output files. This provides a clear example of how to structure and run the pipeline with a specific, larger dataset.

**Using Custom Datasets (e.g., VinDr-Mammo or your own data):**

1.  Structure your dataset similarly to the `sample_data_omi_db/` or `sample_data_vin_dr_mamo/` sample directories provided in this repository (e.g., an `images/` subdirectory containing the mammograms and an initial `exam_list_before_cropping.pkl` file describing the exams).
2.  Modify the path variables at the beginning of `run.sh` (e.g., `DATA_FOLDER`, `INITIAL_EXAM_LIST_PATH`, and various output paths like `CROPPED_IMAGE_PATH`, `EXAM_LIST_PATH`, etc.) to point to your dataset's specific locations.

To facilitate ease of use, this repository includes sample directory structures and example files within `sample_data_omi_db/` and `sample_data_vin_dr_mamo/` (these directories are no longer in `.gitignore`). These samples illustrate the expected input format and the kind of files generated by the pipeline.

## Introduction

This is an implementation of the model used for breast cancer classification as described in our paper [Deep Neural Networks Improve Radiologists' Performance in Breast Cancer Screening](https://ieeexplore.ieee.org/document/8861376). The implementation allows users to get breast cancer predictions by applying one of our pretrained models: a model which takes images as input (_image-only_) and a model which takes images and heatmaps as input (_image-and-heatmaps_).
...
**Update (2019/08/26)**: We have added a [TensorFlow implementation](using_tensorflow.md) of our _image-wise_ model.

**Update (2019/06/21)**: We have included the _image-wise_ model as described in the paper that generates predictions based on a single mammogram image. This model slightly under-performs the _view-wise_ model used above, but can be used on single mammogram images as opposed to full exams.
...
This section outlines the software and library requirements. The original model had a specific set of dependencies, which have been updated in this adapted version for compatibility with newer technologies and to incorporate additional tools like PyTorch Lightning.

### Current Prerequisites (Adapted Version)
...
These dependencies are also reflected in the `requirements.txt` file.

### Original Prerequisites (NYU Model - circa 2019)

The original model, as described in the 2019 paper, had the following prerequisites:
...
Here we describe how to get predictions from _view-wise_ model, which is our best-performing model. This model takes 4 images from each view as input and outputs predictions for each exam.
...
Predictions using _image-only_ model (found in `sample_output/image_predictions.csv` by default):
...
Predictions using _image-and-heatmaps_ model (found in `sample_output/imageheatmap_predictions.csv` by default):
...
Here we also upload _image-wise_ model, which is different from and performs worse than the _view-wise_ model described above. The csv output from _view-wise_ model will be different from that of _image-wise_ model in this section. Because this model has the benefit of creating predictions for each image separately, we make this model public to facilitate transfer learning.

To use the _image-wise_ model, run a command such as the following:
...
- `IMAGE_MODEL_PATH`: The path where the saved weights for the _image-only_ model is saved.
- `IMAGEHEATMAPS_MODEL_PATH`: The path where the saved weights for the _image-and-heatmaps_ model is saved.
...
- `IMAGE_PREDICTIONS_PATH`: The path to save predictions of _image-only_ model.
- `IMAGEHEATMAPS_PREDICTIONS_PATH`: The path to save predictions of _image-and-heatmaps_ model.
